...
if ((!empty($_SESSION['user_id']) || !empty($_SESSION['is_demo'])) && ($_SESSION['role'] ?? '') === 'regulator') {
...
function sendOtpEmail($to_email, $to_name, $otp) {
    ...
    <body style="font-family:Arial,sans-serif;background:#f4f4f4;padding:30px;">
      <div style="max-width:480px;margin:auto;background:#fff;border-radius:10px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        ...
        <p style="color:#555;">Use the code below to sign in to the <strong>Regulator Portal</strong>. It expires in <strong>10 minutes</strong>.</p>
        ...
        <p style="color:#aaa;font-size:12px;">Nagrik Seva · Regulator Portal</p>
      </div>
    </body>
    </html>';
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Your Nagrik Seva Regulator Login OTP';
        $mail->Body    = $html_body;
...
if (isset($_GET['demo'])) {
    $_SESSION['user_id'] = -1;
    $_SESSION['role']    = 'regulator';
    $_SESSION['name']    = 'Adv. Meera Kamat';
    $_SESSION['dept']    = 'Goa Grievance Authority';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login_password') {
        $pw = $_POST['password'] ?? '';
        if ($email === 'demo.regulator@nagrikseva.gov' && $pw === 'Demo@1234') {
            $_SESSION['user_id'] = -1; $_SESSION['role'] = 'regulator';
            $_SESSION['name'] = 'Adv. Meera Kamat'; $_SESSION['dept'] = 'Goa Grievance Authority';
            $_SESSION['is_demo'] = true;
            header('Location: regulator_dashboard.php'); exit;
        }
        if (!$email || !$pw) { $error = 'Please fill in both fields.'; }
        else {
            $st = $conn->prepare("SELECT id,name,role,password_hash,is_active FROM users WHERE email=? AND role='regulator' LIMIT 1");
            $st->bind_param('s', $email); $st->execute();
            $u = $st->get_result()->fetch_assoc(); $st->close();
            if (!$u) $error = 'No regulator account found with that email.';
            elseif (!($u['is_active'] ?? 1)) $error = 'Account inactive. Contact admin.';
            elseif (!password_verify($pw, $u['password_hash'] ?? '')) $error = 'Incorrect password.';
                ...
                $_SESSION['otp_pending_name'] = $u['name'];
        ...
        $mode = 'otp_verify'; $otp_in = trim($_POST['otp'] ?? ''); $omail = $_SESSION['otp_pending_email'] ?? '';
        if (strlen($otp_in) < 6) $error = 'Enter all 6 digits.';
        elseif (!$omail) { $error = 'Session expired. Start again.'; $mode = 'password'; }
        else {
            ...
            if (!$tok) $error = 'No OTP found.';
            elseif (strtotime($tok['expires_at']) < time()) $error = 'OTP expired.';
            elseif ($tok['otp'] !== $otp_in) $error = 'Wrong code.';
            else {
                $del = $conn->prepare("DELETE FROM otp_tokens WHERE email=?");
                $del->bind_param('s',$omail); $del->execute(); $del->close();
                $_SESSION['user_id'] = $_SESSION['otp_pending_uid'];
                $_SESSION['name']    = $_SESSION['otp_pending_name'];
                $_SESSION['email']   = $omail; $_SESSION['role'] = 'regulator';
                $conn->query("UPDATE users SET last_login=NOW() WHERE id=".(int)$_SESSION['user_id']);
                unset($_SESSION['otp_pending_email'],$_SESSION['otp_pending_uid'],$_SESSION['otp_pending_name'],$_SESSION['dev_otp']);
                header('Location: regulator_dashboard.php'); exit;
            }
        }
...
<title>Regulator Login — Nagrik Seva</title>
...
/* LEFT — identical gradient to citizen login */
.lp{width:50%;display:flex;flex-direction:column;justify-content:space-between;padding:52px 56px;overflow:hidden;flex-shrink:0;position:relative;
  background:linear-gradient(160deg,#042e2a 0%,#065449 60%,#18cfb4 100%)!important;
  border-right:1px solid rgba(255,255,255,.12)!important;}
.lp::after{content:'';position:absolute;top:0;right:0;bottom:0;width:1px;background:linear-gradient(to bottom,transparent,rgba(24,207,180,.8),transparent)}
.lp *{color:#ffffff!important}
...
.logo-row:hover .logo-mark{box-shadow:0 6px 28px rgba(24,207,180,.4);transform:scale(1.05);}
.logo-mark{width:44px;height:44px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.2);border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:20px;position:relative;flex-shrink:0;transition:all .2s;box-shadow:0 4px 18px rgba(0,0,0,.2);}
.logo-mark::after{content:'';position:absolute;inset:-3px;border-radius:15px;border:1.5px solid rgba(24,207,180,.35);animation:pr 2.5s ease-in-out infinite}
@keyframes pr{0%,100%{opacity:.7}50%{opacity:.15}}
.logo-n{font-size:1rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;}
.logo-t{font-size:.6rem;opacity:.45;letter-spacing:.4px;margin-top:2px}

.role-badge{display:inline-flex;align-items:center;gap:7px;padding:6px 14px;border-radius:20px;background:rgba(255,255,255,.1)!important;border:1px solid rgba(255,255,255,.18)!important;font-size:.65rem;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;opacity:.85;margin-bottom:18px}
.role-badge::before{content:'';width:6px;height:6px;border-radius:50%;background:#16c9af;animation:blink 1.5s ease-in-out infinite}
@keyframes blink{0%,100%{opacity:1}50%{opacity:.25}}
.hl{font-size:clamp(2.4rem,3.5vw,3.4rem);font-weight:900;line-height:1.0;letter-spacing:-2px;margin-bottom:18px}
.hl em{font-style:normal;color:#adf2e8!important;display:block}
.hl-sub{font-size:.88rem;opacity:.6;line-height:1.8;max-width:330px;margin-bottom:40px}
.duties{display:flex;flex-direction:column;gap:10px}
.duty{display:flex;align-items:center;gap:13px;padding:13px 16px;border-radius:12px;background:rgba(255,255,255,.07)!important;border:1px solid rgba(255,255,255,.1)!important;transition:all .22s;}
.duty:hover{background:rgba(255,255,255,.13)!important;border-color:rgba(24,207,180,.35)!important;transform:translateX(4px)}
.duty-ico{width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;background:rgba(255,255,255,.1)!important;}
.duty-n{font-size:.8rem;font-weight:600}
.duty-d{font-size:.67rem;opacity:.45;margin-top:2px}
.lp-foot{position:relative;z-index:1;font-size:.67rem;opacity:.3}
.other-portals{display:flex;gap:10px;margin-bottom:14px}
.op{padding:6px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.18)!important;background:rgba(255,255,255,.08)!important;font-size:.7rem;font-weight:600;opacity:.65!important;transition:all .2s}
.op:hover{background:rgba(255,255,255,.16)!important;opacity:1!important}
...
<div class="lp">
  ...
  <div class="lp-top">
    <a href="regulator_login.php?demo=1" class="logo-row">
      <div class="logo-mark">🏛️</div>
      <div>
        <div class="logo-n">Nagrik Seva</div>
        <div class="logo-t">Goa Civic Portal · <?= date('Y') ?></div>
      </div>
    </a>
    <div class="role-badge">⚖️ Regulator Portal</div>
    <div class="hl">Oversight.<br>Accountability.<em>Justice.</em></div>
    <div class="hl-sub">The Regulator oversees the entire grievance ecosystem — monitoring officers, enforcing accountability, and ensuring every complaint is resolved.</div>
    <div class="duties">
      <div class="duty"><div class="duty-ico">👁️</div><div><div class="duty-n">Monitor all officers</div><div class="duty-d">See who's active, idle, or underperforming</div></div></div>
      <div class="duty"><div class="duty-ico">🔔</div><div><div class="duty-n">Send performance notices</div><div class="duty-d">Notify idle officers with escalation warnings</div></div></div>
      <div class="duty"><div class="duty-ico">⚖️</div><div><div class="duty-n">Issue legal notices</div><div class="duty-d">Formal notices for continued negligence</div></div></div>
      <div class="duty"><div class="duty-ico">🚫</div><div><div class="duty-n">Terminate accounts</div><div class="duty-d">Remove non-compliant officers from the system</div></div></div>
    </div>
  </div>
  <div class="lp-foot">
    <div class="other-portals">
      <a href="citizen_login.php" class="op">👤 Citizen Login</a>
      <a href="officer_login.php" class="op">👮 Officer Login</a>
    </div>
    © <?= date('Y') ?> Government of Goa · Nagrik Seva
  </div>
</div>
...
<div class="eyebrow">Regulator Portal</div>
<div class="title">Welcome back ⚖️</div>
<div class="sub">Sign in to your oversight dashboard.</div>

<?php if ($error): ?><div class="alert a-err">⚠ <?= htmlspecialchars($error) ?></div><?php endif; ?>

<!-- Demo cheat button -->
<a href="regulator_login.php?demo=1" class="demo-btn">⚡ &nbsp;Quick Demo Login — Skip to Dashboard</a>

<div class="divider"><span></span><p>or sign in manually</p><span></span></div>

<form method="POST">
  <input type="hidden" name="action" value="login_password">
  <div class="fg"><div class="fl"><label>Official Email</label></div><input class="fi" type="email" name="email" placeholder="regulator@nagrikseva.gov" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" required autofocus></div>
...
<div class="bottom">Not a regulator? <a href="officer_login.php">Officer login →</a> · <a href="citizen_login.php">Citizen</a></div>
...
<?php endif; ?>
<?php if ($error):   ?><div class="alert a-err">⚠ <?= htmlspecialchars($error) ?></div><?php endif; ?>
<?php if ($success): ?><div class="alert a-ok">✅ <?= htmlspecialchars($success) ?></div><?php endif; ?>
...
function submitOtp(e){const d=[...document.querySelectorAll('.ob')].map(b=>b.value).join('');if(d.length<6){e.preventDefault();return;}document.getElementById('otp-val').value=d;}
(function(){const btn=document.getElementById('rbtn'),cd=document.getElementById('cd');if(!btn)return;let s=60;const t=setInterval(()=>{s--;cd&&(cd.textContent=s);if(s<=0){clearInterval(t);btn.disabled=false;btn.innerHTML='Resend OTP';}},1000);})();
